...
export default function AIConsultant() {
  ...
  async function handleSend() {
    ...
    // Pega as mensagens ANTES da nova entrar (mas remove a saudação inicial)
    // Pega só as últimas 6 mensagens para poupar tokens do Google
    const recentHistory = messages.filter(m => !m.text.includes("Olá! Sou o Assistente Industrial")).slice(-6)
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        // AGORA ENVIAMOS O HISTÓRICO JUNTO!
        body: JSON.stringify({ 
          message: userText,
